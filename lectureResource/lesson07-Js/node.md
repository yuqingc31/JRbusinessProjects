 ES6是JavaScript语言的下一代标准。目标是来编写复杂的大型应用程序

const:
1.不可重新赋值：使用 const 声明的变量是常量，一旦被赋值，就不能再修改其值。尝试重新赋值给常量会导致错误。

2.块级作用域：与 var 不同，const 声明的变量具有块级作用域，即只在声明它的块内部有效。

3.必须初始化：const 声明的变量必须在声明时进行初始化，即必须赋予一个初始值。

4.对象和数组的限制：尽管 const 声明的变量不能被重新赋值，但对于对象和数组来说，它们内部的属性或元素是可变的。这意味着你可以修改对象或数组的属性或元素，但不能重新赋值给整个对象或数组。
